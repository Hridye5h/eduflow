  ...
  return (
    <>
      ...
      <div className="flex-1 p-6 space-y-5 max-w-3xl">
        ...
        {isAdmin && (
          <Card>
            <CardHeader title={<span className="inline-flex items-center gap-2"><Megaphone className="h-4 w-4 text-[var(--color-brand)]" /> Broadcast</span>} subtitle="Send to everyone or by role." />
            <CardBody>
              <form onSubmit={send} className="space-y-3">
                <div className="grid sm:grid-cols-2 gap-3">
                  ...
                  <Input label="Title" value={broadcast.title} onChange={(e) => setBroadcast({ ...broadcast, title: e.target.value })} required />
                </div>
                <label className="block">
                  <span className="block text-xs font-semibold uppercase tracking-wider text-[var(--color-text-subtle)] mb-1.5">Message</span>
                  <textarea
                ...
                <div className="flex gap-1 p-1 bg-[var(--color-surface-muted)] rounded-lg text-xs">
                  {(['all', 'unread'] as const).map((t) => (
                    <button
                      key={t}
                      onClick={() => setTab(t)}
                      className={`px-3 py-1 rounded-md font-medium transition-colors ${tab === t ? 'bg-white shadow-sm text-[var(--color-text)]' : 'text-[var(--color-text-muted)]'}`}
                    >
                      {t === 'all' ? 'All' : 'Unread'}
                    </button>
                  ))}
            ...
            {items.length === 0 ? (
              ...
              <ul>
                {items.map((n) => (
                  ...
                  >
                    <div className={`h-2 w-2 rounded-full mt-2 shrink-0 ${n.readAt ? 'bg-transparent' : 'bg-[var(--color-brand)]'}`} />
